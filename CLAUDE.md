...
- **ユーザーストーリーの書き方**（`docs/00`）:
  ...
  - **IDは `US-<優先度>-<3桁連番>`**（例 `US-1-002`）。優先度は P0=1 / P1=2 / P2=3。
    **3桁にするのは途中挿入のため**。連番を詰め直すと他ドキュメントからの参照が壊れる。
...
- [ ] **MVP: アプリ → Lambda → AgentCore → Bedrock を繋ぐ** ← **いま最優先**（US-1-001〜3）
- [ ] MVP機能2: 連続取得（watchPositionAsync）→ 2点間から進行方位を算出（US-2-002）
- [ ] 音声化（STT/TTS）（US-1-004/5）
- [ ] PoC: バックグラウンド常駐＋ウェイクワードの実機検証（US-2-003・**最大の技術リスク**）

### いま作っているもの（MVPのスコープ）

**「現在地とともに質問 → AIが調べて答える」をスマホアプリで実現する**（`docs/00_user_stories.md` US-1-001〜4）。
...
> ⚠️ **メモ機能（US-3-001）は将来必要だが今は作らない。** 学習しながらの開発では初手から重い。
