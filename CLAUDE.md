...
| ディレクトリ | 役割 | 書き方 |
|---|---|---|
| **`.planning/`** | **いまの状況**（`issues.md`=決めるべき論点 / `todo.md`=やるべき作業）。セッション引き継ぎの起点 | **下記「進捗と課題の管理」のルールに従う** |
...
**セッションをまたいで引き継ぐための、進捗と課題の置き場。**
Claude Code が自動生成するメモリとは別物。
...
| ファイル | 中身 | 消えるとき |
|---|---|---|
| [.planning/issues.md](.planning/issues.md) | **論点**（判断が必要なこと）。優先度（高/中/低）付き | **決めれば**消える |
| [.planning/todo.md](.planning/todo.md) | **作業**（やることが明確なこと） | **やれば**消える |
...
| **セッションの終わり** | 記述と実態がズレていないか見直し、`最終更新` の日付を直す |
...
- **やらないと決めたものは消す。** 優先度を下げて延命させない。
  要件レベルでやらないと決まったものは `docs/00_user_stories.md` §5 に書く。
- **「完了したもの」「決着した論点」は消さない。** この粒度なら量は増えないので、
  到達点の記録として残す方が有用。
- **判断の経緯や実測値は書かない。** それは `pre-research/` や `docs/` の役目。
  ここには**「何をやるか / 何を決めたか」と「どこに詳細があるか」だけ**を書く。
...
> `.planning/` は「いま動いている状況」。**同じことを二重に書くと必ず片方が古くなる。**
