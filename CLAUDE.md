...
| ディレクトリ | 役割 | 書き方 |
|---|---|---|
| **`.planning/`** | **進捗と課題**（`todo.md`=確定したこと / `issues.md`=未着手の課題）。セッション引き継ぎの起点 | **下記「進捗と課題の管理」のルールに従う** |
...
| ファイル | 何を書くか |
|---|---|
| [.planning/issues.md](.planning/issues.md) | **未着手の課題。** 作業だけでなく**まだ決めていない論点**も含む。優先度（高/中/低）付き |
| [.planning/todo.md](.planning/todo.md) | **やる順番が決まった作業**と、**やり終えたこと**（日付つき・消さない） |

**課題は一方向に流れる。** 逆流や分岐はない。

```
issues.md（未着手）→ todo.md「次にやる」→ todo.md「完了したもの」
```

論点も同じ流れをたどる。「Xをどうするか」（issues）→「Xを決める」（todo）→
**完了時に結論を残す**（完了したもの）。だから**結論専用の置き場は作らない**。
...
| **新しい課題・論点が判明したとき** | `issues.md` に優先度をつけて追加（その場で対処しないなら必ず書く） |
| **着手する順番が決まったとき** | `issues.md` → `todo.md`「次にやる」へ**移す**（両方に書かない） |
| **作業や判断が終わったとき** | `todo.md`「完了したもの」に**日付と結論**を書く |
| **セッションの終わり** | 記述と実態がズレていないか見直し、`最終更新` の日付を直す |

> ⚠️ **「あとでまとめて」にしない。** 会話の途中で判明した課題は、
> **その時点で `issues.md` に書く**（セッションが切れたら失われるため）。
...
- **数を増やしすぎない。** **人が一目で読める量**が上限
  （目安: `todo.md`「次にやる」は3件程度、`issues.md` は10件以内）。
  ⚠️ `issues.md` が溢れたら、**優先度を下げて延命させず「もうやらない」と決めて消す。**
