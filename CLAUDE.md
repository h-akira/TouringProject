...
**`.memory/` → 成果物 の向きにしか参照してはいけない。**

```
.memory/  ──参照OK──▶  docs/ adr/ learning/ pre-research/
          ◀──禁止───
```

| | |
|---|---|
| ✅ `.memory/issues.md` から `docs/01b` を参照する | 道具が成果物を指すのは正しい |
| ❌ `docs/` `adr/` から `.memory/` を参照する | **成果物が道具に依存してしまう** |

**なぜダメか**: `.memory/` はAIが随時書き換え、**やれば消える・決まれば消える**もの。
成果物がそこを指すと、**参照先が消えて壊れる**。
`.memory/` が消えても成果物は無傷、という関係を保つ。

- 未決の事項を `docs/` に書くのは構わない。**ただし「どこで管理しているか」は書かない。**
  「〜は未決」とだけ書き、`.memory/issues.md` へのリンクは張らない。
- ⚠️ **例外は各ディレクトリの `README.md`**（`backend/README.md` 等）。
  あれは開発者向けの手順書なので、「次にやることは `.memory/todo.md` を見ること」でよい。
