...
| **MVPに必要か** | **どちらでも US-1.01〜04 は満たせる** |
...
| 観点 | 判断 |
|---|---|
| MVP（US-1.01〜04）に必要か | **どちらでも満たせる** → 判断材料にならない |
...
> ⚠️ **補足: この「Gatewayは過剰」という評価は、上表のとおり「メモ機能（自分のリソースを叩く）」に限った話。**
> **Web検索（US-1.04）には当てはまらない。** そちらは自前で用意できない外部の索引を使うため、
> Gateway の組み込みコネクタを採用している（[../websearch/](../websearch/)）。
> 「Gatewayは使わない」という一般則として読まないこと。

## 6. 未決事項

- [ ] 認証を **APIキー のままにするか Cognito にするか**（MVPではどちらでも動く。複数ユーザー配布が現実になったら要判断）
