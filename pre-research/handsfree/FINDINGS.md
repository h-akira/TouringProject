...
## 14. ❌ 走行中は音量VADが成立しない（`metering` が 0 dBFS に飽和）
...
## 15. ❌ `audioSource` では解決しない（4種を実測）

**2026-09-09、実機で4種を測り比べた**（App v1.29.0・設定画面から切り替え）。
§14 の飽和が `audioSource` で回避できるかを確かめた。

### 結果

| `audioSource` | ノイズ除去 | 判定 |
|---|---|---|
| `mic`（既定） | — | ❌ |
| `voice_recognition` | — | ❌ |
| **`voice_communication`** | ⚠️ **4種で最も効いた** | ❌ **それでも不足** |
| `unprocessed` | — | ❌ |

**最良だった `voice_communication` の実測:**

| 状況 | `metering` |
|---|---|
| **エンジンON・停止中**（アイドリング） | ✅ **-50 dB 程度** |
| エンジンON・停止中で**発話** | ✅ **-50 より大きくなる**（＝**差が出る**） |
| ⚠️ **アクセルを開けたとき** | ⚠️ **0 dB 付近に達することがある** |

### ⚠️ 結論：使い物にならない

**アイドリング中だけ見れば成立していた。** ノイズ除去が効き、
**黙ったときと喋ったときで値が動いた**（§14 の完全な飽和からは改善している）。

⚠️ **しかし走行には使えない。** **アクセルを開けると飽和する**ため、
**走行中＝アクセルを開けている時間**では、結局 §14 と同じ状態になる。

📌 **回転数に応じて騒音が変わる**ので、**閾値を固定できない**。
低速では成立し高速では破綻する、という**速度依存の不安定さ**は、
⚠️ **走行中に画面を見られない本アプリでは「たまに壊れる」＝使えない**に等しい。

> 📌 **教訓: 「改善した」と「使える」は違う。**
> `voice_communication` は明確に改善したが、**最悪ケース（全開加速）で破綻する**なら、
> ⚠️ **ハンズフリーの前提を満たさない。**

### 帰結

**音量ベースのVADは、`audioSource` の選択を含めて打ち止め。**
⚠️ **端末側の設定で回避できる問題ではないと確定した**（§14 の帰結表に追加）。

📌 **ただし `voice_communication` はノイズ除去が最も効いたので、
録音の品質としては優れている。終話判定を別方式にしても、この設定は残す価値がある。**

## 16. ✅ 録音中でも `VOICE_COMMAND` は届く（実機で確認・App v1.31.0）

**2026-09-10、実機で成立を確認した。**
**録音中にインカムのボタンを押すと Intent が届き、その場で送信された。**

⚠️ **これが方式Cの唯一の未知だった。**
**マイクを掴んでいる最中に Bluetooth スタックが Intent を送るか**は
**端末・インカム側の挙動**なので、⚠️ **コードからは判断できなかった。**

📌 **過去にCを却下した理由の一つが、まさにこの未検証性だった**（[DECISION.md](DECISION.md)）。
**その懸念は実機で否定された。**

### 前提（コードを読んで確認済み・2026-09-09）

| 事実 | 根拠 |
|---|---|
| **`onNewIntent` でも deep link に読み替えている** | `App/plugins/withVoiceInteraction.js`（`onCreate` と両方で `redirectVoiceCommandToAutoRecord` を呼ぶ） |
| **押すたびにURLが変わる** | `autoRecord=<System.currentTimeMillis()>`。同じ文字列だと `useURL()` が再発火しない |
| **アプリ起動中の再配送は成立済み** | 「戻った後、インカムのボタンで再度起動できる」を実機で確認（§13.3） |

⚠️ **つまり未知なのは「録音中か否か」の一点だけ**で、Android 側の配線は既にできている。

### 検証の手順（走行・エンジン不要。USB接続とMacが要る）

```sh
adb logcat | grep -E "handsfree|vad"
```

1. インカムのボタンを押す → アプリが起動して録音が始まる
2. **録音中にもう一度ボタンを押す**
3. ログを見る
...
| ログ | 意味 |
|---|---|
| `[handsfree] VOICE_COMMAND received: recording=true …` | ✅ **届いた。方式Cが成立する** |
| 続けて `[handsfree] second press while recording (<ms>) -> send` | ✅ 送信に進んだ（v1.31.0 で足した分岐） |
| 続けて `[handsfree] second press too soon (<ms>) -> ignore` | ⚠️ **届いてはいる**が1秒未満だったので弾いた（チャタリング対策）。**少し間を置いて押し直す** |
| **何も出ない** | ❌ **届いていない。** ⚠️ **録音中はマイクを掴んでいるため
  Bluetooth スタックが Intent を出さない**ということ。**方式Cは不成立**で、
  B（オンデバイス音声認識）か D 単独に絞られる |

⚠️ **`recording=false` が出た場合は「届いたが録音していなかった」**なので、
手順1が成立していない（別の問題）。
