...
## 13. ❌ `audioSource` では解決しない（4種を実測）

**2026-09-09、実機で4種を測り比べた**（App v1.29.0・設定画面から切り替え）。
§12 の飽和が `audioSource` で回避できるかを確かめた。
...
**黙ったときと喋ったときで値が動いた**（§12 の完全な飽和からは改善している）。

⚠️ **しかし走行には使えない。** **アクセルを開けると飽和する**ため、
**走行中＝アクセルを開けている時間**では、結局 §12 と同じ状態になる。
...
⚠️ **端末側の設定で回避できる問題ではないと確定した**（§12 の帰結表に追加）。
