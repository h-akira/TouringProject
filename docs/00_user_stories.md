...
| ID | ストーリー | 受け入れ条件 |
|---|---|---|
| **US-1-001** | ライダーとして、**今いる場所を踏まえてAIに質問したい**。その土地のことを知りたいから | 現在地が回答に反映される |
| **US-1-002** | ライダーとして、**続けて質問したい**。一問一答では会話にならないから | 指示語（「それ」）が前の質問を受けて解釈される |
| **US-1-003** | ライダーとして、**スマホアプリから使いたい**。バイクに積むのはスマホだから | 実機（Android）で動作する |
| **US-1-004** | ライダーとして、**AIに自分で調べて答えてほしい**。「アプリで検索して」と言われても走行中はできないから | 「調べられません」で終わらず、調べた上で答える |
...
| ID | ストーリー | 備考 |
|---|---|---|
| **US-2-001** | ライダーとして、**声で質問したい**。走行中は入力できないから | STT（Transcribe） |
| **US-2-002** | ライダーとして、**声で回答を聞きたい**。走行中は読めないから | TTS（Polly） |
| **US-2-003** | ライダーとして、**進行方向を踏まえてほしい**。「右手に見える山」と聞きたいから | GPS2点から方位算出 |
| **US-2-004** | ライダーとして、**手を使わずに起動したい**。走行中はスマホに触れないから | ウェイクワード。**最大の技術リスク** |

### P2：将来

| ID | ストーリー | 備考 |
|---|---|---|
| **US-3-001** | ライダーとして、**気になった場所をメモに残したい**。後で見返したいから | 今は作らないが、作れる構造にしておく（補足B） |
| **US-3-002** | ライダーとして、**走った道の記録を残したい**。振り返りたいから | US-3-001と同系統 |
...
- [ ] 「今日の天気は？」に、AIが自分で調べて答える（「アプリで確認して」と返さない）
...
- [ ] ウェイクワードのバックグラウンド常駐が実機で成立するか（**最大のリスク**）
...
**US-3-001（メモ機能）は将来必要**だが、学習しながらの開発では初手から重いため今は作らない。
代わりに**後から足せる構造**にしておく。
