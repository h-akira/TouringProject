...
| **US-2.04** | ライダーとして、**スマホに触れずに起動したい**。走行中の画面操作は法律上できないから | ⚠️ **最大の技術リスク。** 理想はウェイクワードだが、**インカムのボタンでもよい**（補足C） |
...
| 手段 | 可否 |
|---|---|
| **ウェイクワード**（オンデバイス。Porcupine） | ✅ **理想。** 完全に手を使わない |
| **インカムのボタン** | ✅ **許容する。** ハンドルから手を離さずに押せる |
| 画面のボタン | ❌ **不可**（法律） |

> 📌 **利用しているインカムは Bluetooth のマルチファンクションボタンを備える。**
> Android は Bluetooth のボタンを `KEYCODE_MEDIA_*` として配送するため、
> **アプリが受け取れる余地がある**（[Android公式](https://developer.android.com/media/legacy/media-buttons)）。
> ⚠️ **バックグラウンドで受け取れるか**が鍵で、そこが成立しなければ意味がない。
