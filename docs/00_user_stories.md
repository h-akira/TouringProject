...
- [x] **スマホに触れずに一巡できる**（US-2.04。インカムのボタン → 起動 → 録音 →
      **無音検知で自動送信** → 読み上げ → **マップアプリへ復帰**まで実機確認済み。
      [adr/006](../adr/006_handsfree_launch_mechanism.md)・
      [adr/007](../adr/007_return_to_map_after_answer.md)）
      ⚠️ **走行中は成立しない**（エンジン音で音量が飽和し、無音検知が働かない。[01c](01c_app_client.md) §3）
- [ ] 走りながら、上記が通しでできる

## 7. 未確定事項

- [ ] 走行中の騒音下でSTTがどこまで実用になるか（⚠️ **停車中は実用と確認済み**）
- [ ] ⚠️ **走行中の終話判定をどう実現するか**（音量ベースの無音検知は
      **エンジン音による飽和で成立しないと判明**。[01c](01c_app_client.md) §3）
- [ ] コストが現実的な範囲に収まるか
