...
### 4.3 現在日時も Lambda が渡す

**LLMは今日の日付を知らない。** 渡さないと学習時点の感覚で推測し、
**検索で得た新しい記事を「未来の日付」と取り違える。**
天気・営業時間・イベントも日付と時刻に依存するので、**毎回の質問の先頭に添える**
（「【現在日時: 2026年9月26日（土）14時05分（日本時間）】」。`Backend/src/lib/prompt.py` の `format_now`）。

- 位置が無い質問にも付ける（日時は位置に依存しない）。
- エージェントのシステムプロンプトで「日付はこれを正とし、自分で推測しない」と指示する。
- 📌 **分まで渡す。** 時刻はリクエストのログからも分かるので、伏せても記録は減らない。
  **会話の経過時間は別に添える**（AIに時刻どうしの引き算をさせない。[01b](01b_heading.md) §5）。
...
| 文字起こしの出力 | **自分のバケット**（`transcripts/`）。⚠️ **Transcribe用のIAMロールが要る**（下記） |
...
### ⚠️ Transcribe には専用のIAMロールが要る

**文字起こしを書き出すのは、ジョブを開始したLambdaが終了した後**なので、
**Lambdaの権限は使えない**。Transcribe が自分で assume するロールを渡す
（`JobExecutionSettings.DataAccessRoleArn`）。

- 📌 **出力先を指定しなければロールは不要**だが、その場合**AWS管理のバケット**に置かれる。
  ⚠️ **文字起こしはライダーの発話そのもの**であり、そこでは
  **ライフサイクルで消すことも、サポートケースなしに削除することもできない。**
  → **自分のバケットに置く**方を採る。
- 信頼ポリシーには `aws:SourceAccount` / `aws:SourceArn` の条件を付ける
  （**他アカウントからの踏み台化を防ぐ**）。

📌 **ポーリングの打ち切りは120秒**（[01a](01a_async_ask.md) §5）。音声は実測15〜20秒で、
テキスト（10〜13秒）との差が文字起こしのぶん。⚠️ **バッチのジョブ待ちが入れば伸びる**ので、
打ち切りは音声を基準にしてある。
...
| 声 | **`Kazuha`（女性・ニューラル）**。⚠️ 日本語は4声あるが **`Mizuki` はスタンダードのみ**なので選ばない |
...
📌 **実機で認識できることを確認済み**（停車状態）。
⚠️ **走行中の風切り音の下でどうかは未確認**（§12）。
