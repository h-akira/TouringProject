...
| 層 | 対策 |
|---|---|
| **入口** | APIキー必須（§8）。⚠️ **未実装。** 開発中は代わりに**リソースポリシーでIP制限**している |
...
⚠️ **現時点で実装されているのはIP制限だけ。** 認証も流量制限も予算遮断も未実装で、
**IP制限を外すと誰でも叩ける状態になる**（1リクエストごとにBedrockの課金が発生する）。
IP制限は `AllowedIp` パラメータで開発時のみ有効にしている（`backend/README.md`）。
**本番はIPが変わるため使えない**ので、APIキーの実装が事実上の前提条件になる。
