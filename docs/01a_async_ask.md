...
> [01_architecture.md](01_architecture.md) §5 の詳細。全体像はそちらを先に読む。
...
📌 **アプリのポーリング打ち切り（120秒）はこの並びの外側**にある。
⚠️ **揃える必要はない** — アプリが諦めても処理は続き、結果はDynamoDBに残る
（次のポーリングで拾える）。アプリ側は「いつまで待つか」だけを決めている。
...
- 実測はテキスト10〜13秒・**音声15〜20秒**なので、**大半は最初の帯で終わる**。
  ⚠️ **打ち切りは音声を基準にしてある**（文字起こしのぶんが乗り、
  バッチのジョブ待ちが入ればさらに伸びうるため。[01](01_architecture.md) §7）。
