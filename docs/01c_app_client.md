...
**停止と送信は分けない**（走行中に2回操作できない）。**出口は3つ**:

| 出口 | 既定 | 中身 |
|---|---|---|
| **インカムのボタン再押し** | ✅ **有効** | 録音中の `VOICE_COMMAND` を「話し終えた」とみなす。⚠️ **音を一切見ない**。⚠️ **1秒未満の押下は弾く**（チャタリング対策） |
| **`maxRecordingMs` に達する** | ✅ **有効** | 押し忘れの備え。**質問を失わせない** |
| **無音検知（VAD）** | ⚠️ **無効** | 話し終えて静かになったら送る。⚠️ **走行中は成立しない**（下記） |
...
| `audioSource` | **`voice_communication`** | 録音の用途（Androidの `MediaRecorder.AudioSource`）。⚠️ **端末側の音の加工が変わる。** 実測でノイズ除去が最も効いた |
| `useSilenceDetection` | **`false`** | 無音検知を使うか。⚠️ **走行中は成立しないので既定は「使わない」** |
...
### ⚠️ 走行中は無音検知が成立しない（既定で切ってある）

**エンジン始動中は `metering` が 0 dBFS に飽和する。**
...
**そのため `useSilenceDetection` を既定で `false` にしている。**
⚠️ **切ると騒音ガード（下記）も一緒に止まる。**
走行中は「一度も静かにならない」ので、**通すとすべての録音を捨てる**ため。

**無音検知を使わないときの録音の出口は2つ:**

| 出口 | 中身 |
|---|---|
| **インカムのボタンをもう一度押す** | 録音中の `VOICE_COMMAND` を「話し終えた」とみなして送る |
| **`maxRecordingMs` に達する** | 押し忘れても質問が失われない |

📌 **どちらかが必ず効く**ので、押し忘れへの備えになっている。
...
📌 **音量の表示は無音検知が有効なときだけ出す。**
切っているときは閾値が判定に使われないため、⚠️ **出すと効いていない値を見て悩む。**
...
| 騒音で録音を捨てた（§3。⚠️ **無音検知が有効なときだけ**） | 「周囲の音が大きく、聞き取れませんでした。もう一度お話しください。」 |
