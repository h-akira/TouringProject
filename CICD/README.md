...
「`docs/` だけの変更でビルドしたくない」と考えて `FILE_PATH` フィルタを入れたくなるが、
**入れてはいけない。**

⚠️ **CodeBuild の `FILE_PATH` は push の「headコミット」しか見ない。**
そのため**最後のコミットがたまたまドキュメントだけ**だと、
**同じpushに含まれる `Backend/` の変更ごと丸ごとスキップされる。**

実際に起きた（GitHubの実ペイロードで確認）:

| | |
|---|---|
| pushしたコミット数 | 12 |
| うち `Agent/` `Backend/` の変更 | あり |
| **headコミットの変更ファイル** | **6件（すべてドキュメント）** |
| 結果 | ⚠️ **ビルドされず。** Webhookの配信は `200 OK` |

**気づく手段が無いのが最悪**で、「pushしたのにデプロイされていない」状態が
成功に見える。**空振りのビルドより、静かに実行されないデプロイの方がはるかに高くつく。**

📌 **空振りのビルドは安い。** CloudFormation も SAM も
**変更が無ければ何もしない**（`No changes to deploy`）ので、
数分のCodeBuild時間で済む。
