...
## ⚠️ 3つのビルドがある — Expo Go / 開発ビルド / 単体ビルド

**Expo Go を卒業したあと、さらにもう一段の区別が要る。**
⚠️ **「Development Build」と一括りにできない**ので、3つを並べて整理する。

| | **Expo Go** | **開発ビルド** | **単体ビルド（release）** |
|---|---|---|---|
| 正体 | Expo社の既製アプリ | **自分のアプリ** | **自分のアプリ** |
| JSはどこから | Metro | **Metro** | ⚠️ **APKに焼き込み** |
| **Macが必要か** | 必要 | ⚠️ **必要**（無いと起動しない） | ✅ **不要** |
| JS/TSの変更 | 即反映 | **即反映** | ⚠️ **APKを作り直すまで反映されない** |
| ネイティブコード | ❌ 追加不可 | ✅ 追加可 | ✅ 追加可 |
| 使う場面 | （卒業済み） | **開発中** | **実地で使うとき** |

### Metro とは — 「JSの配達係」

`npx expo start` で起動する **Metro bundler** は、⚠️ **Macで動く**開発サーバー。
コードの変更を監視し、**Wi-Fi経由**で実機にJSを届ける。

```
[Mac] コードを保存 → Metro が組み立てる ──Wi-Fi──> [スマホ] 画面を更新
```

📌 **React Nativeのアプリが2層に分かれている**からこそ、これが成立する:

| 層 | 変更したとき |
|---|---|
| **ネイティブ層**（Kotlin・権限・intent-filter） | ⚠️ **再ビルドが必要** |
| **JS層**（`.ts` / `.tsx`） | **Metro経由で即反映** |

### ⚠️ なぜ「単体ビルド」が要るのか

**開発ビルドは Metro が無いと起動しない。**
⚠️ **Macから離れて使うアプリ（本アプリはバイクで使う）では、これでは成立しない。**

そこで **release ビルド**を作る。⚠️ **JSバンドルをAPKの中に焼き込む**ので、
Metroが無くても動く。
...
```sh
unzip -p app-release.apk assets/index.android.bundle | wc -c
# → 0 でなければ焼き込まれている
```

### ⚠️ 落とし穴: スマホに入るのは常に1個

**3つとも同じパッケージ名**なので、⚠️ **後から入れたものが前のものを上書きする。**
**2つ並んで選べるようにはならない。**

📌 **そして見た目で区別できない**（アイコンも名前も同じ）。
⚠️ **画面にバージョン番号を出しておくと、どれが入っているか判別できる。**

```
⚠️ よくある事故:
  単体ビルドが入っている実機で、コードを直しても何も変わらない
  → JSが焼き込まれているので当然。APKを作り直すまで永久に古いまま
```

⚠️ **実際にこれを踏んだ。** 実機のバージョン表示が上がらず、
**コードの変更が届いていないことにしばらく気づけなかった。**

### ⚠️ `app.json` は Metro の管理外

もう一つ紛らわしい点。**`app.json`（バージョン等の設定）は JS ではない**ので、
⚠️ **Metro の起動時に読まれ、そのあと変更しても自動反映されない。**

📌 **`app.json` を変えたら Metro を再起動する。**

### 往復のコストは低い

⚠️ **「単体ビルドを入れたら開発ビルドに戻れない」わけではない。**

- **APKはMacに残る**ので、戻すのは `adb install -r` 1回（数十秒）
- ⚠️ **保存済みの設定も消えない**
  （release が **debugと同じ署名鍵**を使う場合。鍵が違うと上書きできない）

📌 **開発中は開発ビルド、実地で試すときは単体ビルド**、と行き来するのが普通。
